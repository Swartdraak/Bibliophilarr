...
- **NET10-02 migration alignment**: Active build, CI, runtime packaging, and integration harness paths were aligned to the migrated .NET 10 / ASP.NET 10 baseline.
- **Cross-host developer contract**: Documented the supported development hosts (Windows, Linux, macOS Intel/Apple Silicon), the portable console vs Windows desktop entry points, and the RID policy in `QUICKSTART.md` and `CONTRIBUTING.md`. Added a `cross-build-portability.yml` workflow that validates the portable solution on all three hosts and validates the `X64 -> x64` / `Arm64 -> arm64` RID mapping required for Apple Silicon.
- **Direct Windows-desktop project restore**: Removed the `System.Drawing.Common` and `System.Resources.Extensions` direct package references that were breaking `dotnet restore src/NzbDrone/Bibliophilarr.csproj` (release-blocking Rider/Coder project-model regression). Added a scoped transitive-version pin for `System.Drawing.Common` in `Directory.Build.props` so legacy .NET Framework-era packages (PdfPig / MonoTorrent / TagLibSharp-Lidarr) cannot resolve the vulnerable 4.7.0 line (GHSA-rxg9-xrhp-64gj); the 9.0.0 central version pin in `Directory.Packages.props` is now actually enforced.
- **RID policy scoping**: The implicit host RID default in `src/Directory.Build.props` is now scoped to packaging entry points (`WinExe` / `Update`) so framework-dependent developer restore/builds remain portable across hosts.
