...
Current multi-provider enrichment works, but provider-specific field differences can still produce inconsistent aggregate metadata and uncertain precedence behavior across runtime flows.
...
- define precedence for title/subtitle, author identity, identifiers, publication dates, language, and cover links.
