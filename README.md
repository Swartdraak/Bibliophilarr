...
SemVer is injected during CI/release via `BIBLIOPHILARRVERSION` (see `Directory.Build.props`), so `main` and `staging` are represented by release signals while `develop` is represented as the integration lane awaiting promotion/tagging.
