    ...
    public class SymbolicLinkResolver : ISymbolicLinkResolver
    {
        ...
        public string GetCompleteRealPath(string path)
        {
            ...
            try
            {
                ...
                _logger.Warn("Failed to check for symlinks in the path {0}: {1}", LogSanitizer.Sanitize(path), ex.Message);
                return path;
            }
            catch (Exception ex)
            {
                _logger.Debug(ex, "Failed to check for symlinks in the path {0}", LogSanitizer.Sanitize(path));
                return path;
            }
        }
        ...
        private bool TryFollowSymbolicLink(ref string path, out bool wasSymLink)
        {
            ...
            if (link == null)
            {
                ...
                {
                    _logger.Trace("Checking path {0} for symlink returned error {1}, assuming it's not a symlink.", LogSanitizer.Sanitize(path), errno);
                }
