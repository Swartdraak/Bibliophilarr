...
{
    public class BibliophilarrErrorPipeline
    {
        private readonly Logger _logger;

        public BibliophilarrErrorPipeline(Logger logger)
        {
            _logger = logger;
        }
        ...
        public async Task HandleException(HttpContext context)
        {
            ...
            else if (exception is SQLiteException sqLiteException)
            {
                ...
                _logger.Error(sqLiteException, "[{0} {1}]", context.Request.Method, LogSanitizer.Sanitize(context.Request.Path));
            }
            else
            {
                _logger.Fatal(exception, "Request Failed. {0} {1}", context.Request.Method, LogSanitizer.Sanitize(context.Request.Path));
            }
