...
namespace NzbDrone.Core.Download.Clients.DownloadStation
{
    public class TorrentDownloadStation : TorrentClientBase<DownloadStationSettings>
    {
        ...
        public override ProviderMessage Message => new ProviderMessage("Bibliophilarr is unable to connect to Download Station if 2-Factor Authentication is enabled on your DSM account", ProviderMessageType.Warning);
        ...
        protected override string AddFromMagnetLink(RemoteBook remoteBook, string hash, string magnetLink)
        {
            ...
            DsTaskProxy.AddTaskFromUrl(magnetLink, GetDownloadDirectory(remoteBook.ResolvedFormatType), Settings);
        ...
        protected ValidationFailure TestConnection()
        {
            ...
            catch (DownloadClientAuthenticationException ex)
            {
                ...
                {
                    DetailedDescription = $"Please verify your username and password. Also verify if the host running Bibliophilarr isn't blocked from accessing {Name} by WhiteList limitations in the {Name} configuration."
                };
            }
