...
namespace NzbDrone.Core.MetadataSource
{
    ...
    public class MetadataConflictTelemetrySnapshot
    {
        ...
        public Dictionary<string, string> LastDecisionScoreBreakdownByProvider { get; set; }
        ...
        public MetadataConflictTelemetrySnapshot()
        {
            ...
            LastDecisionScoreBreakdownByProvider = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        }
    }
    ...
    public class MetadataConflictTelemetryService : IMetadataConflictTelemetryService
    {
        ...
        private readonly ConcurrentDictionary<string, string> _lastDecisionScoreBreakdownByProvider;
        ...
        public MetadataConflictTelemetryService(Logger logger)
        {
            ...
            _lastDecisionScoreBreakdownByProvider = new ConcurrentDictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            _logger = logger;
        }
        ...
        public void RecordDecision(string operation, MetadataConflictResolutionDecision decision)
        {
            ...
            foreach (var breakdown in decision.ProviderScoreBreakdowns ?? new Dictionary<string, Dictionary<string, int>>())
            {
                var details = breakdown.Value == null
                    ? "none"
                    : string.Join(",", breakdown.Value)
                        .Trim();

                _lastDecisionScoreBreakdownByProvider.AddOrUpdate(
                    breakdown.Key,
                    details,
                    (_, __) => details);
            }
        ...
        public MetadataConflictTelemetrySnapshot GetSnapshot()
        {
            ...
            foreach (var pair in _lastDecisionScoreBreakdownByProvider)
            {
                snapshot.LastDecisionScoreBreakdownByProvider[pair.Key] = pair.Value;
            }
